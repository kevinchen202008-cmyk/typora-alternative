...
function createWindow() {
  ...
  win = new BrowserWindow({
    ...
    webPreferences: {
      ...
      webSecurity:      false,   // allow loading local file:// resources (vditor assets)
    },
  });
  ...
  const template = [
    ...
    },
    {
      ...
      submenu: [
        ...
        { label: 'Replace', accelerator: 'CmdOrCtrl+H', click: () => send('menu-find') },
      ],
    },
    {
      label: 'Paragraph',
...
async function cmdOpenFolder() {
  ...
  if (!r.canceled && r.filePaths[0]) send('menu-open-folder', r.filePaths[0]);
}

// ── IPC handlers ───────────────────────────────────────────────────────────────
ipcMain.handle('get-config',    ()               => config);
ipcMain.handle('update-config', (_, updates)     => { Object.assign(config, updates); writeConfig(config); });
ipcMain.handle('read-file',     (_, p)           => fs.readFileSync(p, 'utf8'));
ipcMain.handle('write-file',    (_, p, data)     => { fs.writeFileSync(p, data, 'utf8'); pushRecent(p); });
...
ipcMain.once('renderer-ready', () => {
  if (pendingFile) { openFile(pendingFile); pendingFile = null; }
});
